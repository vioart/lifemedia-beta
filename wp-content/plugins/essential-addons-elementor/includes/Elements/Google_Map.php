...
?>
		...
		<?php if (!empty($settings['eael_google_map_type'])) : ?>
			...
			<?php if( isset( $settings['enable_marker_search'] ) && 'yes' === $settings['enable_marker_search'] ){
				echo "<div class='eael-google-map-marker-search' id='eael-google-map-" . $this->get_id() ."-search'>";
				echo "<input type='text' id='eael-google-map-" . $this->get_id() ."-markers-input' placeholder='". esc_attr( $settings['marker_search_text'] ) ."'>";
				echo "<ul id='eael-google-map-" . $this->get_id() ."-markers'></ul>";
				echo "</div>";
			} ?>
